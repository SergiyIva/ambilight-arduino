...
void get_colors(Display *d, unsigned char *values, unsigned t, struct config *cnf) {
	...
	for (int k = 0; k < cnf->leds_on_top; k++) {
	int i = cnf->leds_on_side - k - 1;
	
		fillRGB(i * 3,
				cnf->horizontal_pixel_gap + i * cnf->pixels_per_led_top, cnf->horizontal_pixel_gap + (k+1) * cnf->pixels_per_led_top,
	...
	for (int i = 0; i < cnf->leds_on_top; i++) {
		fillRGB((cnf->leds_on_top + cnf->leds_on_side + i) * 3,
	...
	for (int i = 0; i < cnf->leds_on_side; i++) {
		fillRGB((cnf->leds_on_top * 2 + cnf->leds_on_side * 2 - i - 1) * 3,
